  ...
  return (
    <div>
      <div className="flex items-end gap-4">
        <div>
          <div className="font-mono text-5xl font-bold leading-none text-[#3ddc84]">
            {waste.rescued}
          </div>
          <div className="mt-1.5 text-xs text-[#93a3a6]">
            ingredient{waste.rescued === 1 ? "" : "s"} saved from the bin
          </div>
        </div>

        <div className="ml-auto text-right">
          <div className="font-mono text-lg font-semibold text-[#ff5d5d]">{waste.lost}</div>
          <div className="text-[10px] uppercase tracking-widest text-[#61706f]">binned</div>
        </div>

        {waste.save_rate !== null && (
          <div className="text-right">
            <div className="font-mono text-lg font-semibold text-[#eef3f3]">{waste.save_rate}%</div>
            <div className="text-[10px] uppercase tracking-widest text-[#61706f]">save rate</div>
          </div>
        )}
      </div>

      {waste.recent.length > 0 && (
        <p className="mt-3 mb-0 text-xs leading-relaxed text-[#61706f]">
          Lately:{" "}
          {waste.recent
            .map((item) =>
      ...
      <div className="mt-5 border-t border-[#2a3438] pt-4">
        <p className="m-0 mb-2.5 text-[10px] uppercase tracking-widest text-[#61706f]">
          Sample households
        </p>
          ...
          {leaderboard.map((row) => (
            <li
              ...
              className={`flex items-center gap-3 rounded-lg px-2.5 py-1.5 text-sm ${
                row.is_you ? "bg-[rgba(86,217,200,0.12)] text-[#56d9c8]" : "text-[#93a3a6]"
              }`}
            >
              <span className="w-4 font-mono text-xs text-[#61706f]">{row.position}</span>
